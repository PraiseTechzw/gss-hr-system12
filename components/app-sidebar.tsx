...
export function AppSidebar() {
  ...
  // Create navigation with dynamic badges and role-based access
  ...
  useEffect(() => {
    const getUser = async () => {
      ...
      if (user) {
        ...
        console.log('Sidebar user profile fetch:', { profile, profileError, userEmail: user.email })
        
        if (profileError) {
          console.error('Error fetching user profile in sidebar:', profileError)
          // Set a default profile if user exists in auth but not in user_profiles
          setUserProfile({
            role: 'admin', // Default to admin for existing auth users.
            email: user.email || '',
    ...
    // Load favorite pages from localStorage
    ...
    <div className={cn(
              ...
              {userProfile?.role && (
                <div className="flex items-center gap-1 mt-1">
                  ...
                  <span className={cn(
                    ...
                    {userProfile.role?.replace('_', ' ').split(' ').map((word: string) => word.charAt(0).toUpperCase() + word.slice(1)).join(' ') || 'HR'}
                  </span>
                </div>
              )}
            </div>
      ...
      <nav className="flex-1 overflow-y-auto p-4 space-y-6">
        {/* Quick Actions */}
          ...
          {!isCollapsed && <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Quick Actions</h3>}
        ...
        {favoriteNavigation.length > 0 && (
          <div className="space-y-2">
            {!isCollapsed && <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Favorites</h3>}
            <div className="space-y-1">
              {favoriteNavigation.map((item) => {
                ...
                return (
                  ...
                  >
                    {item?.icon && <item.icon className="h-5 w-5 flex-shrink-0 group-hover:scale-110 transition-transform" />}
                  ...
                  >
                      ...
                      <>
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            ...
                            {(item as any).isAdmin && (
                              <Badge variant="destructive" className="text-xs flex-shrink-0 bg-red-100 text-red-800 hover:bg-red-100">
                    ...
                    {isCollapsed && (
                      <div className="absolute left-full ml-2 px-2 py-1 bg-gray-900 text-white text-xs rounded-md opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none z-50 whitespace-nowrap">
                        {item.name}
                        {item.badge && (
                          <span className="ml-1 bg-gray-700 px-1 rounded text-xs">
                            {item.badge}
                          </span>
                        )}
                        {(item as any).isAdmin && (
        ...
        {isAdmin && adminFilteredNavigation.length > 0 && (
          <div className="space-y-2">
            {!isCollapsed && <h3 className="text-xs font-semibold text-red-600 uppercase tracking-wider flex items-center gap-1">
              <Shield className="h-3 w-3" />
              Admin Only
            </h3>}
            <div className="space-y-1">
