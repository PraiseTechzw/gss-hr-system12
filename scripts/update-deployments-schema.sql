...
-- First, let's check what columns currently exist and add the missing ones
...
SET 
    ...
    site_location = COALESCE(site_location, location), -- Use existing location field
...
-- Add constraints for data validation (using DO blocks to handle IF NOT EXISTS)
DO $$ 
BEGIN
    -- Add date constraint
    IF NOT EXISTS (
...
-- (This handles the case where the old 'location' column needs to be renamed)
DO $$ 
BEGIN
    ...
    IF EXISTS (
        ...
        -- Optionally drop the old location column (uncomment if you want to remove it)
